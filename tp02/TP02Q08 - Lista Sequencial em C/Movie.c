...
void list_insertBegin(Movie x) {
    ...
    for(int i = n; i > 0; i--) movies[i] = movies[i-1];
...
void list_insert(Movie x, int pos) {

    if(n >= MAX_MOVIES || (pos < 0 || pos > n)) {

        printf("Insert error: %s!", n >= MAX_MOVIES ? "MAX_MOVIES reached!" : "Invalid position!");
        exit(1);
    }

    for(int i = n; i > pos; i--) movies[i] = movies[i-1];

    movies[pos] = x;
    n++;
}

Movie list_removeBegin() {

    Movie resp;

    if(n == 0) {

        printf("Remove error: Empty stack!");
        exit(1);
    }
    ...
    for(int i = 0; i < n; i++) movies[i] = movies[i+1];
    return resp;
}

Movie list_removeEnd() {
...
Movie list_remove(int pos) {

    Movie resp;

    if(n >= MAX_MOVIES || (pos < 0 || pos > n)) {

        printf("Insert error: %s!", n == 0 ? "Empty stack!" : "Invalid position!");
        exit(1);
    }
    ...
    for(int i = pos; i < n; i++) movies[i] = movies[i+1];
    return resp;
}
...
int main() {
    ...
    for(int i = 0; i < ops; i++) {
        ...
        char line[300], movieName[100], pos[3];

        scanf(" %[^\n]", line);
        strcpy(movieName, substring(line, 4, strlen(line)));
        ...
        if(line[0] == 'I') {
            ...
            if(line[1] == 'I') {
                ...
                list_insertBegin(movie);
            }
            else if(line[1] == 'F') {

                movie_readHtml(&movie, movieName);
                list_insertEnd(movie);
            }
            else if(line[1] == '*') {

                strcpy(pos, substring(line, 4, 2));
                strcpy(movieName, substring(line, 7, strlen(line)));

                movie_readHtml(&movie, movieName);
                list_insert(movie, atoi(pos));
            }
        }
        else if(line[0] == 'R') {
            ...
            if(line[1] == 'I')  printf("(R) %s\n", list_removeBegin().name);
                ...
                strcpy(pos, substring(line, 4, 2));
                printf("(R) %s\n", list_remove(atoi(movieName)).name);
            }
        }
    }
