...
char *movie_getOriginalLanguage(Movie *movie) { return movie -> original_language; }
...
void list_insertByColor(Movie *array, int n, int cor, int h) {
    ...
    for (int i = (h + cor); i < n; i += h) {
        ...
        while((j >= 0) && (strcmp(movie_getOriginalLanguage(&array[j]), movie_getOriginalLanguage(&tmp)) > 0)) {
...
void list_shellsort() {
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++) list_insertByColor(movies, n, cor, h);
    ...
    fptr = fopen("753045_shellsort.txt", "w+");
