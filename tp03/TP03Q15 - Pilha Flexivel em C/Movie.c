...
Movie stack_remove() {
    ...
    if(top == NULL) errx(1, "x Remove error: Empty stack!");

    Movie resp = top -> element;
...
int stack_size() {
    ...
    for(i = top; i != NULL; i = i -> next) count++;
    return count;
}

void print_c(int j, Cell* i) {
    
    j--;

    if(i -> next != NULL) print_c(j, i -> next);
    
    printf("[%i] ", j);
    movie_print(&(i -> element));
}

void stack_print() { print_c(stack_size(), top); }
...
int main() {
    ...
    for(int i = 0; i < ops; i++) {
        ...
        char line[300], movieName[100];

        scanf(" %[^\n]", line);
        strcpy(movieName, substring(line, 3, strlen(line)));
