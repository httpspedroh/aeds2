...
int getBiggest() {
    ...
    Movie maior = movies[0];

    for (int i = 0; i < n; i++) {

        if(maior.duration < movies[i].duration) maior.duration = movies[i].duration;
    }
    return maior.duration;
}

void list_countingSort() {
    
    clock_t t_begin = clock();

    // ------------------------------------------- //

    int tamCount = getBiggest(movies, n) + 1;
    int count[tamCount];
    int c_compares = 0;

    Movie ordenado[n];

    for(int i = 0; i < tamCount; count[i] = 0, i++) c_compares++;
    for(int i = 0; i < n; count[movies[i].duration]++, i++) c_compares++;
    for(int i = 1; i < tamCount; count[i] += count[i-1], i++) c_compares++;
    for(int i = n-1; i >= 0; ordenado[count[movies[i].duration]-1] = movies[i], count[movies[i].duration]--, i--) c_compares++;
    for(int i = 0; i < n; movies[i] = ordenado[i], i++) c_compares++;

    // -------------- //

    FILE *fptr;

    fptr = fopen("753045_countingsort.txt", "w+");
