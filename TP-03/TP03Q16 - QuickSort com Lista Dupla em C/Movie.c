...
// Functions - Flexible double list
typedef struct DoubleCell {

	Movie element;
	struct DoubleCell* next;
   	struct DoubleCell* prev;

} DoubleCell;

DoubleCell* newDoubleCell(Movie element) {

	DoubleCell* nova = (DoubleCell*)malloc(sizeof(DoubleCell));

	nova -> element = element;
	nova -> prev = nova -> next = NULL;
	return nova;
}
...
void list_start() {
	...
	first = newDoubleCell(invalid);
	last = first;
}

void list_insertEnd(Movie x) {
	
	last -> next = newDoubleCell(x);
	last -> next -> prev = last;
	last = last -> next;
}
...
void list_print() {
    ...
    for(i = first -> next; i != NULL; i = i -> next) movie_print(&(i -> element));
}
...
void list_sort() { 
    ...
    int size = list_size(), j = 0;

    DoubleCell* array[size];
...
void quicksort(DoubleCell** array, int low, int high) {

    int i = low, j = high;

    Movie pivot = array[(low + high) / 2] -> element;

    while(i <= j) {

        if(strcmp(array[i] -> element.situation, pivot.situation) == 0) {

            while(strcmp(array[i] -> element.name, pivot.name) < 0) i++;
        }
        else {

            while(strcmp(array[i] -> element.situation, pivot.situation) < 0) i++;
        }

        if(strcmp(array[j] -> element.situation, pivot.situation) == 0) {

            while(strcmp(array[j] -> element.name, pivot.name) > 0) j--;
        }
        else {

            while(strcmp(array[j] -> element.situation, pivot.situation) > 0) j--;
        }
